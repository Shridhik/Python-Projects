    ...
    if isflush == 0:        # if the rank is not a flush
        ...
        for c in rstr:      # traverse through every eleiment in rank string
            if c == '4':            # if a rank occurs 4 times, it will result in 4 of a kind
                ...
                pos = rstr.index(c)                   # get position of that rank
                have1 = 1                             # change have1 variable to 1
            else:
                pos = 0                               # else position is 0 and have1 variable set to 0
                have1 = 0
        for i in range(pos, pos+5):                   # from that position
            if int(rstr[i]) == 1:
                cons += int(rstr[i])                  # add consecutive 1's
        if have1 == 1 and pos <= 8 and cons == 5:     # if have1 is true and position is less than 8(T) and consecutive 1's in next 5 elements in rank string it is a straight
            rank = 6
            srank = "Straight"
        elif have1 == 1 and pos == 0 and cons == 4 and rstr[12] == '1':  # same as above but for A,2,3,4,5 is also straight
            rank = 6
            srank = "Straight"
    print(rank)
    print(srank)
...
b = ['JC', 'QC', 'KH', 'KC', 'TS', '5C', 'AC', '9H', '8C', '7S']     # test hand to run and check program
...
def playloop(cred, hand):                   # play loop for the video poker (using module VideoPoker.py) with arguments credit and the hand
    ...
    print(lheld)                            # prints the held cards
    ...
    print(rankcount(hand[:5]))
    print(rst)
    print(suitcount(hand[:5]))
    print(sst)
