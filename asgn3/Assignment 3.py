...
MX = maxi(ST)  # variable ST is passed to function maxi() since we need the start value again
...
def mul_walk(WK):                           # function for multiple walks (WK - user input of no of walks)
    j = 0                                   #initialize j which will be used to start the in the while loop
    hit_all = 0                             #these are all used to initialize variables that we will use
    hit_min = 0
    hit_max = 0
    hit_5000 = 0
    mean_ows_all = 0
    ...
    while j < WK:                                           # iterate until max no of walks user input (WK) #main function that runs everything
        ows = one_walk_steps(ST, MN, MX, RH)                #all information from previous dunctions
        ...
        mean_ows_all += delta_all/hit_all                   #knew how to calculate the mean before hand
        var_ows_all += delta_all*(abs(ows) - mean_ows_all)  # variance
        if hit_all > 1:
            sdv_ows_all = ((var_ows_all / (hit_all - 1)) ** 0.5) # equation found on wikipedia site for Algorithims for calculating Variance #was hinted at website during secondary lab, https://en.wikipedia.org/wiki/Algorithms_for_calculating_variance
        elif hit_all <= 1:
            sdv_ows_all = None
    ...
    print("Number of walks that ended on the left:", )              # prints everything out
...
mul_walk(WK)          # runs the main function
